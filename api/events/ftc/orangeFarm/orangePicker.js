...
function orangePickerSeason(season, oranges){
	...
	var pickerTimer = new Date()
	mongodb.db.collection('seasons').findOne(
		{_id: season},
		function(err, pickedOranges){
			console.log('Operation orangePickerSeason time(Milliseconds):',new Date(new Date()-pickerTimer).getMilliseconds())
			console.log('[DONE]-orangePickerSeason')
			if(err)
				console.log(err)
			else
				oranges(pickedOranges)	//Thses oranges don't need peeling;	this is the call back.
...
function orangePickerRanking(orchard, oranges){
	...
	// 	}
		mongodb.db.collection('matchData').aggregate([
			{$match:{'_id.eventKey':orchard}},
			{$lookup:{
				from:'schedules',
				let: {matchNumber: '$_id.matchInformation.matchNumber', eventKey: '$_id.eventKey'},
				pipeline: [
					{$match:{$expr:
						{$eq: ['$_id', '$$eventKey']}
					}},
...
function orangePickerAverageScores(orchard, oranges){
	...
	// 	}
		...
		mongodb.db.collection('schedules').aggregate([
			...
			{$lookup:{
				from:'matchData',
				let: {matchNumber: '$schedule.matchNumber', eventKey: '$_id'},
				pipeline: [
					{$match:{$expr:{$and:[
						{$eq: ['$_id.eventKey', '$$eventKey']},
						{$eq: ['$_id.matchInformation.matchNumber', '$$matchNumber']}
					]}}}
				],
				// localField:'schedule.match',
